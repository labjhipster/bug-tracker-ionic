...
export class LabelUpdatePage implements OnInit {
    ...
    ngOnInit() {
        this.ticketService.query()
            .subscribe(data => { this.tickets = data.body; }, (error) => this.onError(error));
    ...
    protected subscribeToSaveResponse(result: Observable<HttpResponse<Label>>) {
        result.subscribe((res: HttpResponse<Label>) => this.onSaveSuccess(res), (res: HttpErrorResponse) => this.onError(res.error));
    }
    ...
    async onSaveSuccess(response) {
        ...
        toast.present();
        this.navController.navigateBack('/tabs/entities/label');
    }
